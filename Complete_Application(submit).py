...
def add_new_book():
    ...
    try:
        ...
        if existing_book:
            # If the book exists, increment the bookCount
...
def register_user():
    ...
    try:
        ...
        # Check if the email ID (contactDetails) already exists in the database
        cur.execute("SELECT * FROM Users WHERE contactDetails=%s", (contact_details,))
        existing_user = cur.fetchone()
        if existing_user:
            return jsonify({"error": "The provided email ID is already registered."}), 400

        # Insert new user
        cur.execute("INSERT INTO Users (name, contactDetails) VALUES (%s, %s)",
...
def add_to_cart():
    ...
    cart = cur.fetchone()
    if not cart:
        print("No cart found for Cart ID:", cart_id)  # Debugging line
        connection.close()
...
def checkout():
    ...
    try:
        ...
        # Get all books from the specified cart
        ...
        for item in cart_items:
            ...
            if book and book[0] > 0:
                ...
                # Add to Transactions
                cur.execute("INSERT INTO Transactions (userID, bookID, borrowDate) VALUES (%s, %s, NOW())",
                            (user_id, book_id))
            else:
                raise Exception("Book with ID {} is not available.".format(book_id))
